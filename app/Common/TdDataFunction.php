...
//        var_dump($channels);die;
        ...
        $versions = array();
        if ($groupby == 'appversion') {
            $versionInfo = self::getVersionList($gameinfo['app_key']);
            if ($versionInfo['status'] != 200) {
                self::saveErrorLog($versionInfo['message']);
                return;
            }
            foreach ($versionInfo['result'] as $ver) {
                if (!empty($version)) {
                    if (!in_array($ver['versionname'], $version)) {
                        continue;
                    }
                }
                $versions[] = $ver;
...
//        var_dump($versions);die;
        ...
        foreach ($channels as $kk => $d) {
            ...
            foreach ($versions as $ver) {
                ...
                if ($groupby == 'appversion') {
                    $search['filter']['versions'] = array($appversion);
                }
...
//                var_dump($table);die;
                if ($table == "talkingdata_china_keepuser") {
                    ...
                    $sql = "INSERT INTO $table (tjdate,app_id,app_key,app_name,channel_id,channel_name,appversion,versionname,newuser,activeuser) VALUES('$day','{$gameinfo['app_id']}','{$gameinfo['app_key']}','{$gameinfo['app_name']}','$channelid','$channelname','$appversion','{$ver['versionname']}','$newuser','$activeuser') ON DUPLICATE KEY UPDATE newuser='$newuser',activeuser='$activeuser' ";
...
//                var_dump($new_result);
    ...
    // todo 新代码
    public static function getNewBaseDataGroup($gameinfo, $day)
    {
...
//                var_dump($new_result);
    ...
    public static function getBaseDataGroup_orther($gameinfo, $day)
    {
        $table = "talkingdata_china_user";
        $platform_id = $gameinfo['platform_id'];
        $country_type = $gameinfo['country_type'];
