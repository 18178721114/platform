...
class BaiduTgHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'BaiduTgHandleProcesses {dayid?} {data_account?}';
...
//        define('MYSQL_TABLE_NAME','zplay_tg_report_daily');
        ...
        self::BaiduDataProcess($dayid,$source_id,$source_name);
    }

    private static function BaiduDataProcess($dayid,$source_id,$source_name){
        //查询pgsql 的数据
        $map =[];
        $map['dayid'] = $dayid;
        $map['type'] = 2;
...
//            DataImportImp::saveDataErrorLog(2,$source_id,$source_name,4,$error_msg);
        ...
        foreach ($info as $k => $v) {
            ...
            if ($num){
                if ($json_info['account_id'] && $json_info['campaign_id']){
                    $new_campaign_ids[$json_info['account_id']][] = $json_info['campaign_id'];
                }
                $error_log_arr['campaign_id'][] = $json_info['campaign_id'].'('.$third_app_account.'/'.$json_info['account_id'].')';
            }
...
//            }
            ...
            $array[$k]['ad_id'] = isset($json_info['adgroup_id']) ? addslashes($json_info['adgroup_id']) : '';
            $array[$k]['ad_name'] = isset($json_info['adgroup_name']) ? addslashes(str_replace('\'\'','\'',$json_info['adgroup_name'])) : '';
            ...
            $array[$k]['new'] = isset($json_info['activeconversions']) ? $json_info['activeconversions']: 0.00; // 流水原币
...
//        var_dump($new_campaign_ids);
        if ($new_campaign_ids){
            ...
            foreach ($new_campaign_ids as $package_name => $offer_id){
                ...
                $mintegral_conf_info = DB::table('c_generalize')->select(['id','platform_id','account_id'])->where(['platform_id'=> $source_id, 'account_id' => $package_name])->first();
