...
class TiktokTgHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'TiktokTgHandleProcesses {dayid?} {data_account?}';
...
//        define('MYSQL_TABLE_NAME','zplay_tg_report_daily');
    ...
    public static function TiktokDataProcess($dayid,$source_id,$source_name){
        static $tiktok_num = 0;
        //查询pgsql 的数据
        $map =[];
        $map['dayid'] = $dayid;
...
//                    var_dump($new_campaign_ids);
            ...
            foreach ($country_info as $country_k => $country_v) {
                if( (isset($json_info['country_id']) && $json_info['country_id'] == $country_v['name']) ){
            ...
            if(($num+$num_country)>0){
                ...
                $error_detail_arr[$k]['money'] = isset($json_info['stat_cost']) ?$json_info['stat_cost']: 0.00; // 流水原币
            ...
            $array[$k]['impression'] = isset($json_info['show_cnt']) ? $json_info['show_cnt'] : 0;
            $array[$k]['new'] = isset($json_info['convert_cnt']) ? $json_info['convert_cnt'] : 0;
            $array[$k]['click'] = isset($json_info['click_cnt']) ? $json_info['click_cnt'] : 0;

            $array[$k]['cost'] = isset($json_info['stat_cost']) ?$json_info['stat_cost']: 0.00; // 流水原币

            $currency_ex = floatval($ex_info['currency_ex']);
            if (!$currency_ex){
                $currency_ex = 1;
            }
            $array[$k]['cost_exc'] = isset($json_info['stat_cost']) ? $json_info['stat_cost'] * $currency_ex : 0;
