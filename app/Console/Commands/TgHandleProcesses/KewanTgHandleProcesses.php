...
class KewanTgHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'KewanTgHandleProcesses {dayid?} {data_account?}';
...
//        $data_account = $this->argument('data_account') ? $this->argument('data_account'): '717F4ECD-D04A-2DE8-706A-5641D412C940';
//
//        if (!$data_account){
...
//        $map['account'] = $data_account;
        ...
        foreach ($info as $k => $v) {
            ...
            if(($num+$num_country+$num_adtype)>0){
                ...
                $error_detail_arr[$k]['first_level_id'] = $kewan_app_id;
                $error_detail_arr[$k]['first_level_name'] = '';
                $err_ad_id = isset($json_info['ad_id']) ? addslashes($json_info['ad_id']) : '';
                $err_ad_name = isset($json_info['name']) ? addslashes(str_replace('\'\'','\'',$json_info['name'])) : '';
                $error_detail_arr[$k]['second_level_id'] = ''; // $err_ad_id;
                $error_detail_arr[$k]['second_level_name'] = $err_ad_name;
            ...
            $array[$k]['date'] = $dayid;
            $array[$k]['platform_app_id'] = isset($v['app_id']) ? addslashes($v['app_id']) : '';
            $array[$k]['platform_app_name'] = isset($v['app_name']) ? addslashes(str_replace('\'\'','\'',$v['app_name'])) : '';
            $array[$k]['ad_id'] = isset($json_info['ad_id']) ? addslashes($json_info['ad_id']) : '';
            $array[$k]['ad_name'] = isset($json_info['name']) ? addslashes(str_replace('\'\'','\'',$json_info['name'])) : '';

            $array[$k]['impression'] = isset($json_info['imp']) ? $json_info['imp'] : 0;
