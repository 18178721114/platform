...
class AdwodsHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'AdwodsHandleProcesses {dayid?} {data_account?}';
...
//        define('MYSQL_TABLE_NAME','zplay_tg_report_daily');
       ...
       // $data_account = $this->argument('data_account') ? $this->argument('data_account'): 'promtion@zplay.com';
...
//            DataImportImp::saveDataErrorLog(2,$source_id,$source_name,4,$error_msg);
    ...
    private static function AdwordsDataProcess($dayid, $source_id, $source_name){
        ...
        $sql = "SELECT  distinct
                ...
                WHERE 
                c_generalize.platform_id = 'ptg40'";
        ...
        foreach ($info as $k => $v) {
            ...
            $err_name = (isset($json_info['campaign_id']) ? $json_info['campaign_id'] : 'Null') . '#' . (isset($json_info['campaign']) ? addslashes($json_info['campaign']) : 'Null') . '#' . (isset($v['app_id']) ? $v['app_id'] : 'Null') . '#' . (isset($json_info['account']) ?$json_info['account'] : 'Null');
...
//        		    $campaign_os_name = explode('_',$json_info['campaign']);
//        		    var_dump($campaign_os_name);die;
                    //广告位配置未配置
                    $num++;
...
//                var_dump($campaign_os_list);
                ...
                if (in_array('ios',$campaign_os_list)){
                    $app_os_id = 1;
                }elseif(in_array('and',$campaign_os_list)){
                    $app_os_id = 2;
                }elseif(in_array('android',$campaign_os_list)){
                    $app_os_id = 2;
                }elseif(in_array('h5',$campaign_os_list)){
                    $app_os_id = 3;
                }if(in_array('amazon',$campaign_os_list)){
                    $app_os_id = 4;
                }
...
//                        var_dump($app_info_detail[0]['id']);
                        $new_campaign_ids[$third_app_id][$app_info_detail[0]['id']][$json_info['campaign_id']] = $json_info['campaign'];
                    }
//                    var_dump($new_campaign_ids);
            ...
            // todo 匹配国家用
       	    foreach ($country_info as $country_k => $country_v) {
                if(isset($json_info['location']) && $json_info['location'] == $country_v['name']){
            ...
            if ($num_country){
                $error_log_arr['location'][] = (isset($json_info['location']) ? $json_info['location'] : 'Unknown Region').'('.$err_name.')';
            }
...
//            $array[$k]['country_id'] = 16;
            ...
            if(($num+$num_country)>0){
                ...
                $error_detail_arr[$k]['first_level_id'] = $third_app_id ? $third_app_id : '';
                $error_detail_arr[$k]['first_level_name'] = '';
                $err_campaign_id = isset($json_info['campaign_id']) ? addslashes($json_info['campaign_id']) : '';
                $err_campaign = isset($json_info['campaign']) ? addslashes(str_replace('\'\'','\'',$json_info['campaign'])) : '';
                $error_detail_arr[$k]['second_level_id'] = ''; //$err_campaign_id;
                $error_detail_arr[$k]['second_level_name'] = ''; //$err_campaign;
                $error_detail_arr[$k]['money'] = isset($json_info['cost']) ? $json_info['cost'] / 1000000: 0.00; // 流水原币
                $error_detail_arr[$k]['account'] = $third_app_account ? $third_app_account : '';
            ...
            $array[$k]['cost_exc'] = isset($json_info['cost']) ? $json_info['cost'] * $currency_ex / 1000000: 0;
...
//                            var_dump(222,$offer_id_nums);
                        foreach ($offer_id_nums as $offer_id_nums_key => $offer_id_nums_value) {
                            ...
                            $insert_generalize_ad_info['campaign_id'] = strval($offer_id_nums_key);
                            $insert_generalize_ad_info['campaign_name'] = $offer_id_nums_value;
...
//                var_dump($app_info);
        ...
        if ($error_log_arr){
            ...
            if (isset($error_log_arr['location'])){
