...
class GdtTgHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'GdtTgHandleProcesses {dayid?} {data_account?}';
...
//        define('MYSQL_TABLE_NAME','zplay_tg_report_daily');
        ...
        self::GdtDataProcess($source_id,$source_name,$dayid);

    }


    private static function GdtDataProcess($source_id,$source_name,$dayid){
        //查询pgsql 的数据
        $map =[];
        $map['dayid'] = $dayid;
        $map['type'] = 2;
...
//            DataImportImp::saveDataErrorLog(2,$source_id,$source_name,4,$error_msg);
        ...
        $sql = "SELECT  distinct
                ...
                WHERE 
                c_generalize.platform_id = 'ptg80'";
        ...
        foreach ($info as $k => $v) {
            ...
            if ($num){
                if ($json_info['account'] && $json_info['campaign_id']){
                    $new_campaign_ids[$json_info['account']][] = $json_info['campaign_id'];
                }
                $error_log_arr['campaign_id'][] = $json_info['campaign_id'].'('.$third_app_account.'/'.$json_info['account'].')';
            }
            ...
            if(($num+$num_country)>0){
                ...
                $err_app_name = isset($json_info['app_name']) ? addslashes($json_info['app_name']) : '';

                $err_campaign_id = isset($json_info['campaign_id']) ? addslashes($json_info['campaign_id']) : '';
                $err_campaign_name = isset($json_info['campaign_name']) ? addslashes($json_info['campaign_name']) : '';

                $error_detail_arr[$k]['first_level_id'] = $json_info['account']; // $err_app_id;
                ...
                $error_detail_arr[$k]['money'] = isset($json_info['cost']) ? ($json_info['cost'] / 100) : 0.00; // 流水原币
            ...
            $array[$k]['platform_app_name'] = isset($json_info['app_name']) ? addslashes($json_info['app_name']) : '';

            $array[$k]['ad_id'] = isset($json_info['campaign_id']) ? addslashes($json_info['campaign_id']) : '';
            $array[$k]['ad_name'] = isset($json_info['campaign_name']) ? addslashes($json_info['campaign_name']) : '';
            ...
            $array[$k]['cost'] = isset($json_info['cost']) ? ($json_info['cost'] / 100) : 0.00; // 流水原币

            $currency_ex = floatval($ex_info['currency_ex']);
            if (!$currency_ex){
                $currency_ex = 1;
            }
            $array[$k]['cost_exc'] = isset($json_info['cost']) ? ($json_info['cost'] * $currency_ex / 100) : 0;
...
//        var_dump($new_campaign_ids);
        if ($new_campaign_ids){
            ...
            foreach ($new_campaign_ids as $package_name => $offer_id){
                ...
                $mintegral_conf_info = DB::table('c_generalize')->select(['id','platform_id','data_account'])->where(['platform_id'=> $source_id, 'data_account' => $package_name])->first();
...
//        var_dump($error_log_arr);
        ...
        if ($error_log_arr){
            ...
            // 发送邮件
//            CommonFunction::sendMail($error_msg_array,$source_name.'推广平台数据处理error');
        }
