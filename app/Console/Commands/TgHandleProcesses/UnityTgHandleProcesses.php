...
class UnityTgHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'UnityTgHandleProcesses {dayid?} {data_account?}';
...
//            DataImportImp::saveDataErrorLog(2,$source_id,$source_name,4,$error_msg);
        ...
        $sql = "SELECT  distinct
                ...
                WHERE 
                c_generalize.platform_id = 'ptg36' ";
        ...
        foreach ($info as $k => $v) {
        	...
        	if(($num+$num_country+$num_adtype)>0){
                ...
                $err_target_store_id = isset($json_info['target_store_id']) ? addslashes($json_info['target_store_id']) : '';
                $err_target_name = isset($json_info['target_name']) ? addslashes(str_replace('\'\'','\'',$json_info['target_name'])) : '';
                ...
                $error_detail_arr[$k]['first_level_id'] = $err_target_store_id;
                $error_detail_arr[$k]['first_level_name'] = $err_target_name;
        	...
        	$array[$k]['date'] = $dayid;
        	$array[$k]['platform_app_id'] = isset($json_info['target_store_id']) ? addslashes($json_info['target_store_id']) : '';
        	$array[$k]['platform_app_name'] = isset($json_info['target_name']) ? addslashes(str_replace('\'\'','\'',$json_info['target_name'])) : '';
        ...
        if ($error_log_arr){
            ...
            if (isset($error_log_arr['target_store_id']) && !empty($error_log_arr['target_store_id'])){
                sort($error_log_arr['target_store_id']);
                ...
                $error_msg_mail[] = 'target_store_id匹配失败，ID为：'.$target_store_id;
            }
