...
class TwitterTgHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'TwitterTgHandleProcesses {dayid?}';
...
//        define('MYSQL_TABLE_NAME','zplay_tg_report_daily');
        ...
        self::twitterTgDataProcess($dayid, $source_id, $source_name);
    }


    private static function twitterTgDataProcess($dayid, $source_id, $source_name){
        try {
            static $twitter_num = 0;
            //查询pgsql 的数据
            $map = [];
            $map['dayid'] = $dayid;
            $map['type'] = 2;
...
//                DataImportImp::saveDataErrorLog(2,$source_id,$source_name,4,$error_msg);
            ...
            foreach ($info as $k => $v) {
                ...
                $err_name = (isset($json_info['campaign_id']) ? $json_info['campaign_id'] : 'Null') . '#' . (isset($json_info['campaign_name']) ? addslashes($json_info['campaign_name']) : 'Null') . '#' . (isset($v['app_id']) ? $v['app_id'] : 'Null') . '#' . (isset($json_info['app_name']) ? $json_info['app_name'] : 'Null');
                ...
                foreach ($country_info as $country_k => $country_v) {
                    if ((isset($json_info['country_id']) && ($json_info['country_id'] == $country_v['name'])) || (isset($json_info['country_id']) && ($json_info['country_name'] == $country_v['name'])))
                        {
                ...
                $array[$k]['ad_id'] = '';
                $array[$k]['ad_name'] = isset($json_info['placement']) ? $json_info['placement'] : '';
