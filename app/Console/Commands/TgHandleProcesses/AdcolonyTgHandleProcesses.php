...
class AdcolonyTgHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'AdcolonyTgHandleProcesses {dayid?}';
...
//        define('MYSQL_TABLE_NAME','zplay_tg_report_daily');
    ...
    private static function AdcolonyDataProcess($dayid,$source_id,$source_name){
        ...
        $sql = "SELECT  distinct
                ...
                WHERE 
                c_generalize.platform_id = 'ptg63' ";
        ...
        foreach ($info as $k => $v) {
            ...
            $third_app_id = $json_info['store_id'];
            ...
            if(($num+$num_country)>0){
                ...
                $error_detail_arr[$k]['money'] = $json_info['spend'];
                $error_detail_arr[$k]['account'] = $data_account ? $data_account : '';
            ...
            $array[$k]['click'] = isset($json_info['total_clicks']) ? $json_info['total_clicks'] : 0;
        ...
        if ($error_log_arr){
            ...
            if (isset($error_log_arr['campaign_id'])){
                $offer_id = implode(',',array_unique($error_log_arr['campaign_id']));
                $error_msg_array[] = 'campaign_id匹配失败,ID为:'.$offer_id;
                $error_msg_mail[] = 'campaign_id匹配失败，ID为：'.$offer_id;
            }
