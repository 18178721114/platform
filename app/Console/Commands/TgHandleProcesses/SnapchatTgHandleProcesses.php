...
class SnapchatTgHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'SnapchatTgHandleProcesses {dayid?} {data_account?}';
        ...
        foreach ($info as $k => $v) {
            ...
            foreach ($app_list as $app_k => $app_v) {
                if(isset($json_info['campaign_id']) && ($json_info['campaign_id'] == $app_v['campaign_id'])){
                    ...
                    $platform_account = '';
                    $account_map = [];
                    $account_map['agency_platform_id'] = $app_v['agency_platform_id'];
                    $plat_agency_list = PlatformLogic::getPlatformAgenceMapping($account_map)->get();
                    $plat_agency_list = Service::data($plat_agency_list);
                    if ($plat_agency_list){
                        $plat_agency_account_ids = [];
                        foreach ($plat_agency_list as $plat_agency){
                            $plat_agency_account_ids[] = $plat_agency['platform_account_id'];
                        }
                        if ($plat_agency_account_ids){
                            $plat_account_map = [];
                            $plat_account_map['in'] = ['id',$plat_agency_account_ids];
                            $plat_account_map['platform_id'] = $source_id;
                            $plat_account_map['data_account'] = $app_v['data_account'];
                            $plat_agency_account_list = PlatformLogic::getPlatformAccountMapping($plat_account_map)->first();
                            $plat_agency_account_list = Service::data($plat_agency_account_list);
                            if ($plat_agency_account_list){
                                $platform_account = $plat_agency_account_list['account'];
                            }
                        }
                    }

                    $array[$k]['platform_account'] = $platform_account ? $platform_account : $app_v['data_account'];
...
//                var_dump($json_info['campaign_id']);
                ...
                $campaign_os_old_name = explode('-',$json_info['campaign_name']);
                $campaign_os_old_list = array_map('strtolower',$campaign_os_old_name);
//                var_dump($campaign_os_list);
                $app_os_id = '';
                if (in_array('ios',$campaign_os_list) || in_array('ios',$campaign_os_old_list)){
                    $app_os_id = 1;
                }elseif(in_array('and',$campaign_os_list) || in_array('and',$campaign_os_old_list)){
                    $app_os_id = 2;
                }elseif(in_array('android',$campaign_os_list) || in_array('android',$campaign_os_old_list)){
                    $app_os_id = 2;
                }elseif(in_array('h5',$campaign_os_list) || in_array('h5',$campaign_os_old_list)){
                    $app_os_id = 3;
                }if(in_array('amazon',$campaign_os_list) || in_array('amazon',$campaign_os_old_list)){
                    $app_os_id = 4;
                }
...
//                    var_dump($new_campaign_ids);
            ...
            if(($num+$num_country+$num_adtype)>0){
                ...
                $err_adaccount_id = isset($json_info['adaccount_id']) ? addslashes($json_info['adaccount_id']) : '';
                $err_adaccount_name = isset($json_info['adaccount_name']) ? addslashes(str_replace('\'\'','\'',$json_info['adaccount_name'])) : '';

                $error_detail_arr[$k]['first_level_id'] = $third_app_id; // $err_adaccount_id;
                $error_detail_arr[$k]['first_level_name'] = $err_adaccount_name;
                $error_detail_arr[$k]['second_level_id'] = ''; // $json_info['campaign_id'];
                $error_detail_arr[$k]['second_level_name'] = $json_info['campaign_name'];
                $error_detail_arr[$k]['money'] = isset($json_info['spend']) ? $json_info['spend']/1000000 : 0.00; // 流水原币
            ...
            $array[$k]['date'] = $dayid;
            $array[$k]['platform_app_id'] = isset($json_info['adaccount_id']) ? addslashes($json_info['adaccount_id']) : '';
            $array[$k]['platform_app_name'] = isset($json_info['adaccount_name']) ? addslashes(str_replace('\'\'','\'',$json_info['adaccount_name'])) : '';
            ...
            $array[$k]['click'] = isset($json_info['swipes']) ? $json_info['swipes'] : 0;
            $array[$k]['new'] = isset($json_info['total_installs']) ? $json_info['total_installs'] : 0;

            $array[$k]['cost'] = isset($json_info['spend']) ? $json_info['spend']/1000000 : 0.00; // 流水原币

            $currency_ex = floatval($ex_info['currency_ex']);
            if (!$currency_ex){
                $currency_ex = 1;
            }
            $array[$k]['cost_exc'] = isset($json_info['spend']) ? $json_info['spend']/1000000 * $currency_ex : 0;
