...
class VungleTgHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'VungleTgHandleProcesses {dayid?} {data_account?}';
...
//        define('MYSQL_TABLE_NAME','zplay_tg_report_daily');
    ...
    private static function VungleDataProcess($dayid,$source_id,$source_name){
        ...
        $sql = "SELECT  distinct
                ...
                WHERE 
                c_generalize.platform_id = 'ptg37'";
        ...
        foreach ($info as $k => $v) {
            ...
            if(($num+$num_country)>0){
                ...
                $error_detail_arr[$k]['first_level_name'] = $third_app_id;

                $err_campaign_id = isset($json_info['campaign id']) ? addslashes($json_info['campaign id']) : '';
                $err_campaign_name = isset($json_info['campaign name']) ? addslashes(str_replace('\'\'','\'',$json_info['campaign name'])) : '';
            ...
            $array[$k]['platform_app_id'] = isset($json_info['campaign id']) ? addslashes($json_info['campaign id']) : '';
            $array[$k]['platform_app_name'] = isset($json_info['campaign name']) ? addslashes(str_replace('\'\'','\'',$json_info['campaign name'])) : '';

            $array[$k]['ad_id'] = isset($json_info['creative id']) ? addslashes($json_info['creative id']) : '';
            $array[$k]['ad_name'] = isset($json_info['creative name']) ? addslashes(str_replace('\'\'','\'',$json_info['creative name'])) : '';
