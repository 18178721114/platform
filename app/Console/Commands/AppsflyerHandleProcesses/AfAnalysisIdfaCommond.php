...
class AfAnalysisIdfaCommond extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = ' AfAnalysisIdfaCommond {dayid?} {hours?}';
    /**
...
//        define('MYSQL_TABLE_NAME','zplay_tg_report_daily');
        ...
        $map['dayid'] = $dayid;
        $map['hours'] = $hours;
        $fields = ['countrycode','state','city','postalcode','advertisingid','idfa','imei','appid','mediasource','ip'];
        $info = DataImportLogic::getChannelData('appsflyer_pull_data','erm_data',$map,$fields)->get();
        $info = Service::data($info);
        if(!$info){
            $error_msg = $dayid.'号，'.$source_name.'推广平台af-pull数据处理程序获取原始数据为空';
        ...
        if(!$app_list){
            $error_msg = $source_name.'推广平台af-pull数据处理程序应用数据查询为空';
            DataImportImp::saveDataErrorLog(2,$source_id,$source_name,4,$error_msg);
            exit;
        }
        //匹配平台
        ...
        foreach ($info as $k => $v) {
            ...
            if(empty($v['advertisingid']) && empty($v['idfa'])&& empty($v['imei']))  continue ;
            ...
            if ($num){
                $error_log_arr['appid'][] = $v['appid'];
            }
            $array[$k]['platform_id']=$v['mediasource'];
            // todo 匹配平台
            foreach ($platform_match_info as $platform_k => $platform_v) {
                if( (isset($v['mediasource']) &&  strtoupper($v['mediasource']) == strtoupper($platform_v['source_name']))){
            ...
            if ($num_platform>0){
                $error_log_arr['platform_id'][] = $v['mediasource'];
            }
            ...
            $array[$k]['advertising_id'] = isset($v['advertisingid']) ? addslashes($v['advertisingid']) : '';
            ...
            $array[$k]['postal_code'] = isset($v['postalcode']) ? addslashes($v['postalcode'])  : '';
            $array[$k]['country_code'] = isset($v['countrycode']) ? addslashes($v['countrycode']): '';
