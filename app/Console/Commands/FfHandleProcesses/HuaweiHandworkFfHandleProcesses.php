...
class HuaweiHandworkFfHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'HuaweiHandworkFfHandleProcesses {dayid?} ';
        ...
        try {
            foreach ($info as $k => $v) {
            	...
            	foreach ($app_list as $app_k => $app_v) {
                        if($json_info['应用名称'] ==$app_v['billing_app_name']  ){
                ...
                if($num){
                    $error_log_arr['app_id'][]=$json_info['应用名称'];
                }
                ...
                $array[$k]['pay_time'] = $json_info['付费笔数'];
                $array[$k]['pay_user_all'] = $json_info['付费用户数'];
                $array[$k]['pay_time_all'] = $json_info['付费笔数'];
                $array[$k]['earning'] = $json_info['付费金额（元）']*$ex_info['currency_ex'];
                $array[$k]['earning_fix'] =$json_info['付费金额（元）']*$ex_info['currency_ex'];//流水人民币
            	$array[$k]['earning_divide_plat'] = $json_info['付费金额（元）']*$ex_info['currency_ex']*$divide_ad;//流水平台分成
                $array[$k]['earning_divide_plat_pay'] = $json_info['付费金额（元）']*$ex_info['currency_ex']*$bad_account_rate;//流水坏账
            	$array[$k]['earning_divide_publisher'] = $json_info['付费金额（元）']*$ex_info['currency_ex']*(1-$divide_ad);

            	$array[$k]['income_plat'] = $json_info['付费金额（元）']*$ex_info['currency_ex']*$divide_ad;
            	$array[$k]['income_publisher'] = $json_info['付费金额（元）']*$ex_info['currency_ex']*(1-$divide_ad);
            	$array[$k]['income_fix'] =$json_info['付费金额（元）']*$ex_info['currency_ex']*(1-$divide_ad);
