...
class XiaomiHandworkFfHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'XiaomiHandworkFfHandleProcesses {dayid?} ';
        ...
        try{
            foreach ($info as $k => $v) {
                ...
                foreach ($app_list as $app_k => $app_v) {
                        if($json_info['应用ID'] ==$app_v['billing_app_id']  ){
                ...
                if(($num)>0){
                    ...
                    $error_detail_arr[$k]['money'] = floatval($json_info['米币付费金额'])+floatval($json_info['微信付费金额'])+floatval($json_info['支付宝付费金额'])+floatval($json_info['其他付费金额']);
                ...
                //$user_num = floatval($json_info['米币付费用户数'])+floatval($json_info['微信付费用户数'])+floatval($json_info['支付宝付费用户数'])+floatval($json_info['其他付费用户数']);
                $array[$k]['business_id'] = $json_info['应用ID'];
                $array[$k]['business_name'] = $json_info['应用名称'];
                $array[$k]['pay_user'] = 0;
                $array[$k]['pay_time'] = 0;
                $array[$k]['pay_user_all'] = 0;
                $array[$k]['pay_time_all'] = 0;
                //$money = floatval($json_info['米币付费金额'])+floatval($json_info['微信付费金额'])+floatval($json_info['支付宝付费金额'])+floatval($json_info['其他付费金额']);
                $money = floatval($json_info['付费总金额']);
                ...
                $array[$k]['earning_divide_plat_pay'] = $money*$ex_info['currency_ex']*$bad_account_rate;//流水坏账
                $array[$k]['earning_divide_publisher'] = $money*$ex_info['currency_ex']*(1-$divide_ad);

                $array[$k]['income_plat'] = $money*$ex_info['currency_ex']*$divide_ad;
                $array[$k]['income_publisher'] = $money*$ex_info['currency_ex']*(1-$divide_ad);
                $array[$k]['income_fix'] =$money*$ex_info['currency_ex']*(1-$divide_ad);
