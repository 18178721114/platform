...
class WechatFfHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'WechatFfHandleProcesses {dayid?} ';
    ...
    public function handle()
    {
        ...
        if(!$info){
//            $error_msg = $dayid.'号，微信计费平台数据处理程序获取原始数据为空';
//            echo $error_msg;
        ...
        try {
            foreach ($info as $k => $v) {
            	...
            	foreach ($app_list as $app_k => $app_v) {
                    if($json_info['gameid'] ==$app_v['gameid']  ){
                        $array[$k]['app_id'] = $app_v['new_app_id'];
                        $num = 0;
                        break;
                    }else{
                        $num++;
                ...
                if($num){
                    $error_log_arr['app_id'][] = $json_info['gameid'].'('.$json_info['appid'].')';
                }
                ...
                foreach ($country_info as $country_k => $country_v) {
                    ...
                    if(strtolower($json_info['country']) == strtolower($country_v['name']) ){
            	...
            	if(($num+$num_country+$num_channel)>0){
                    ...
                    $error_detail_arr[$k]['first_level_id'] = $json_info['gameid'];
                    $error_detail_arr[$k]['first_level_name'] = $json_info['appid'];
                    $error_detail_arr[$k]['second_level_id'] = ''; // $json_info['channelid'];
                    $error_detail_arr[$k]['second_level_name'] = '';
                    $error_detail_arr[$k]['money'] = $json_info['total_fee'];
...
//                $array[$k]['pay_time_all'] = $json_info['quantity'];
                $array[$k]['earning'] = $json_info['total_fee']*$ex_info['currency_ex'];
                $array[$k]['earning_fix'] =$json_info['total_fee']*$ex_info['currency_ex'];//流水人民币
            	$array[$k]['earning_divide_plat'] = $json_info['total_fee']*$ex_info['currency_ex']*$divide_ad;//流水平台分成
                $array[$k]['earning_divide_plat_pay'] = $json_info['total_fee']*$ex_info['currency_ex']*$bad_account_rate;//流水坏账
            	$array[$k]['earning_divide_publisher'] = $json_info['total_fee']*$ex_info['currency_ex'];
            	$array[$k]['income_plat'] = $json_info['total_fee']*$ex_info['currency_ex']*$divide_ad;
            	$array[$k]['income_publisher'] = $json_info['total_fee']*$ex_info['currency_ex'];
            	$array[$k]['income_fix'] =$json_info['total_fee']*$ex_info['currency_ex'];
            	$array[$k]['remark'] = 'api接口数据';
