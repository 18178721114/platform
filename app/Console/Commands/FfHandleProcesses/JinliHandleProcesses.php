...
class JinliHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'JinliHandleProcesses {dayid?} ';
    ...
    public function handle()
    {
        ...
        var_dump(count($info));
        if(!$info){
//            $error_msg = $dayid.'号,金立计费平台数据处理程序获取原始数据为空';
//            echo $error_msg;
        ...
        try {
            foreach ($info as $k => $v) {
            	...
            	if(($num)>0){
                    ...
                    $error_detail_arr[$k]['money'] = floatval(str_replace('￥','',$json_info['income']));
                ...
                $array[$k]['pay_user'] = $json_info['pay_orders'];
                $array[$k]['pay_time'] = $json_info['pay_orders'];
                $array[$k]['pay_user_all'] = $json_info['pay_orders'];
                $array[$k]['pay_time_all'] = $json_info['pay_orders'];
                $income = floatval(str_replace('￥','',$json_info['income']));
                $array[$k]['earning'] = $income*$ex_info['currency_ex'];
                $array[$k]['earning_fix'] =$income*$ex_info['currency_ex'];//流水人民币
            	$array[$k]['earning_divide_plat'] = $income*$ex_info['currency_ex']*$divide_ad;//流水平台分成
                $array[$k]['earning_divide_plat_pay'] = $income*$ex_info['currency_ex']*$bad_account_rate;//流水坏账
            	$array[$k]['earning_divide_publisher'] = $income*$ex_info['currency_ex']*(1-$divide_ad);

            	$array[$k]['income_plat'] = $income*$ex_info['currency_ex']*$divide_ad;
            	$array[$k]['income_publisher'] = $income*$ex_info['currency_ex']*(1-$divide_ad);
            	$array[$k]['income_fix'] =$income*$ex_info['currency_ex']*(1-$divide_ad);
