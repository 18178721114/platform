...
class VivoHandworkFfHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'VivoHandworkFfHandleProcesses {dayid?} ';
        ...
        try {
            foreach ($info as $k => $v) {
            	...
            	if(($num)>0){
                    ...
                    $error_detail_arr[$k]['money'] = $json_info['付费总额'];
                ...
                $array[$k]['earning'] = $json_info['付费总额']*$ex_info['currency_ex'];
                $array[$k]['earning_fix'] =$json_info['付费总额']*$ex_info['currency_ex'];//流水人民币
            	$array[$k]['earning_divide_plat'] = $json_info['付费总额']*$ex_info['currency_ex']*$divide_ad;//流水平台分成
                $array[$k]['earning_divide_plat_pay'] = $json_info['付费总额']*$ex_info['currency_ex']*$bad_account_rate;//流水坏账
            	$array[$k]['earning_divide_publisher'] = $json_info['付费总额']*$ex_info['currency_ex']*(1-$divide_ad);

            	$array[$k]['income_plat'] = $json_info['付费总额']*$ex_info['currency_ex']*$divide_ad;
            	$array[$k]['income_publisher'] = $json_info['付费总额']*$ex_info['currency_ex']*(1-$divide_ad);
            	$array[$k]['income_fix'] =$json_info['付费总额']*$ex_info['currency_ex']*(1-$divide_ad);
