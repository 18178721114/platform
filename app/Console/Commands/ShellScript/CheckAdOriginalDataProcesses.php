...
class CheckAdOriginalDataProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'CheckAdOriginalDataProcesses {begin_date?}  {end_date?}';
    ...
    public function handle()
    {
        ...
        WHERE
        (type =2 or type  = 3)
        and  dayid between '$begin_date' and '$end_date' and income !=0  group by dayid,source_id  HAVING  sum(income)>0";
        ...
        if ($pgsql_info &&  $pgsql_info_test) {
            foreach ($pgsql_info as $key => $value) {
                foreach ($pgsql_info_test as $k => $v) {
                    if($value['dayid'] == $v['dayid'] && $value['source_id'] == $v['source_id'] && $value['income'] < $v['income'] ){
                        $arr_1[$v['dayid'].'_'.$v['source_id']] = $v['source_id'].'-'.$value['income'].'-'.$v['income'];
            ...
            foreach ($arr_1 as $plat_k => $plat_v) {
                foreach ($info_plat as $plat_k_1 => $plat_v_1) {
                    ...
                    if($plat_date[1] == $plat_v_1['platform_id'] ){
                        $sel_sql = "SELECT count(1) as count from  ad_data.erm_data where dayid = '$plat_date[0]' and source_id = '$plat_date[1]'";
                        $count = DB::connection('pgsql')->select($sel_sql);
                        $count = Service::data($count);
                        //var_dump($count);
                        if($count[0]['count']){
                            $delete_sql = "delete from ad_data.erm_data where dayid = '$plat_date[0]' and source_id = '$plat_date[1]'";
                            DB::connection('pgsql')->delete($delete_sql);

                        }
                        $sel_sql = "SELECT * from  ad_data.erm_data where income !=0  and dayid = '$plat_date[0]' and source_id = '$plat_date[1]'";
                        //echo  $sel_sql;
