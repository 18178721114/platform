...
class CheckTgProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'CheckTgProcesses {dayid?} ';
    ...
    public function handle()
    {
        ...
        if(count($plat_date)>0){
            ...
            if(count($error_plat)>0){
                ...
                $str =rtrim('推广抓回原始数据与处理过后的数据原币大于20的错误平台和日期:,'.$str,',');
                DataImportImp::saveDataErrorLog(3,'推广原始数据与处理数据对数','推广原始数据与处理数据对数',6,$str);
                $date =date('Y-m-d');
        ...
        if(count($plat_date_show)>0){
            ...
            if(count($error_plat)>0){
                ...
                DataImportImp::saveDataErrorLog(3,'推广处理数据与显示数据对数','推广处理数据与显示数据对数',6,rtrim($str,','));
                $date =date('Y-m-d');
                $error_msg_mail[0]='推广处理数据与显示数据对数人民币大于100的错误平台和日期:'.$str;
                // 发送邮件
                CommonFunction::sendMail($error_msg_mail,$date.'数据不准平台和时间信息');
            }
    ...
    public function check_pgsql_mysql($BeginDate,$EndDate,$platform_id=''){
        ...
        //var_dump($pgsql_info);
        //mysql 数据求和；
        $mysql_sql ="select sum(cost) as cost ,platform_id,date from zplay_tg_report_daily where date BETWEEN '$BeginDate' AND '$EndDate' $mysql_where GROUP BY platform_id,date";
