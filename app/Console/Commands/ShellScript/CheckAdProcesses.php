...
class CheckAdProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'CheckAdProcesses {dayid?} ';
    ...
    public function handle()
    {
        ...
        //检查原始数据 和 处理过后的数据
        $plat_date =self::check_pgsql_mysql($BeginDate,$EndDate);
        // 如果 原始数据 减 处理过后 大于范围 跑处理过程
        if(count($plat_date)>0){
            $error_plat = [];
            $num = 0;
            foreach ($plat_date as $key => $value) {
                    $platform_id = $value['platform_id'];
                    $date = $value['date'];
                    $platform_sql = "select platform_id,commond_name from c_data_commond where type = 2 and platform_id='$platform_id' ";
                    $platform_name = DB::SELECT($platform_sql);
                    $platform_name =Service::data($platform_name);
                    $id = Artisan::call($platform_name[0]['commond_name'],['dayid'=>$date]); 
            ...
            if(count($error_plat)>0){
                ...
                $str =rtrim('广告抓回原始数据与处理过后的数据原币大于20的错误平台和日期:,'.$str,',');
                DataImportImp::saveDataErrorLog(3,'广告原始数据与处理数据对数','广告原始数据与处理数据对数',6,$str);
                $date =date('Y-m-d');
                $error_msg_mail[0]=$str;
                // 发送邮件
        ...
        if(count($plat_date_show)>0){
            ...
            if(count($error_plat)>0){
                ...
                DataImportImp::saveDataErrorLog(3,'广告处理数据与显示数据对数','广告处理数据与显示数据对数',6,rtrim($str,','));
                $date =date('Y-m-d');
                $error_msg_mail[0]='广告处理数据与显示数据对数人民币大于100的错误平台和日期:'.$str;
                // 发送邮件
                CommonFunction::sendMail($error_msg_mail,$date.'数据不准平台和时间信息');
            }
    ...
    public function check_pgsql_mysql($BeginDate,$EndDate,$platform_id=''){
        if($platform_id!=''){
            $pgsql_where = " and source_id  = '$platform_id'";
        ...
        ad_data.erm_data
        WHERE
        dayid BETWEEN '$BeginDate'
        AND '$EndDate'
        and  income <>0  $pgsql_where
        GROUP BY
        source_id,
        dayid ";
        ...
        //var_dump($pgsql_info);
        //mysql 数据求和；
        $mysql_sql ="select sum(earning) as earning ,platform_id,date from zplay_ad_report_daily where date BETWEEN '$BeginDate' AND '$EndDate' $mysql_where GROUP BY platform_id,date";
        ...
        foreach ($pgsql_info as $key => $value) {
            foreach ($mysql_info as $k => $v) {
                if($value['source_id'] ==$v['platform_id'] && $value['dayid'] ==$v['date'] ){
                    if($value['income']-$v['earning']>20){
                        $plat_date[$i]['platform_id']=$v['platform_id'];
                        $plat_date[$i]['date']=$v['date'];
                        $plat_date[$i]['diff']=$value['income']-$v['earning'];
                        $i++;
                    }
                }
