...
class KedaxunfeiHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'KedaxunfeiHandleProcesses {dayid?} ';
    ...
    {
        $source_id='pad29';
        $source_name='Kedaxunfei';
        set_time_limit(0);
        $dayid = $this->argument('dayid')?$this->argument('dayid'):date('Y-m-d',strtotime('-1 day'));
        var_dump('Kedaxunfei广告-pad29-'.$dayid);
        //查询pgsql 的数据
        $map =[];
        $map['dayid']  =$dayid;
        $map['type']  =2;
        ...
        if(!$info){
//            $error_msg = $dayid.'号，Kedaxunfei广告平台数据处理程序获取原始数据为空';
//            DataImportImp::saveDataErrorLog(2,$source_id,'Kedaxunfei',2,$error_msg);
            exit;
        }
        ...
        if(!$app_list){
        	$error_msg = $dayid.'号，Kedaxunfei广告平台数据处理程序应用数据查询为空';
            DataImportImp::saveDataErrorLog(2,$source_id,'Kedaxunfei',2,$error_msg);
            exit;
        }
        ...
        if(!$country_info){
            $error_msg = $dayid.'号，Kedaxunfei广告平台数据处理程序国家信息数据查询为空';
            DataImportImp::saveDataErrorLog(2,$source_id,'Kedaxunfei',2,$error_msg);
            exit;
        }
        ...
        foreach ($info as $k => $v) {
        	...
        	foreach ($app_list as $app_k => $app_v) {
        		if($json_info['adunit_id'] ==$app_v['ad_slot_id'] ){
         ...
         //    }
        	...
        	if(($num+$num_country+$num_adtype)>0){
                ...
                $err_appid = addslashes($json_info['appid']);
                $err_app_name = addslashes(str_replace('\'\'','\'',$json_info['app_name']));
                $err_adunit_id = addslashes($json_info['adunit_id']);
                $err_adunit_name = addslashes(str_replace('\'\'','\'',$json_info['adunit_name']));
                $error_detail_arr[$k]['first_level_id'] = ''; // $err_appid;
                $error_detail_arr[$k]['first_level_name'] = $err_app_name;
                $error_detail_arr[$k]['second_level_id'] = $err_adunit_id;
                $error_detail_arr[$k]['second_level_name'] = $err_adunit_name;
            ...
            $array[$k]['ad_unit_id'] = addslashes($json_info['adunit_id']);
        	$array[$k]['ad_unit_name'] = addslashes(str_replace('\'\'','\'',$json_info['adunit_name']));
