...
class OppoHandworkHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'OppoHandworkHandleProcesses {dayid?} ';
        ...
        try { 
            foreach ($info as $k => $v) {
            	...
            	if(($num)>0){
                    ...
                    $error_detail_arr[$k]['money'] = $json_info['收入'];
            	...
            	$array[$k]['impression'] = $json_info['展示'];
            	$array[$k]['click'] = $json_info['点击'];
            	$array[$k]['earning'] = $json_info['收入'];
                if($app_list[0]['divide_ad']){
                    $divide_ad = 1-$app_list[0]['divide_ad']/100;
                }else{
                    $divide_ad =1;
                }
            	$array[$k]['earning_exc'] = $json_info['收入']*$divide_ad;
                $array[$k]['earning_flowing'] =isset($json_info['收入']) ? $json_info['收入']: 0;
            	$array[$k]['earning_fix'] = $json_info['收入']*$divide_ad;
