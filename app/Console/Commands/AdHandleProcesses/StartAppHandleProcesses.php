...
class StartAppHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'StartAppHandleProcesses {dayid?} ';
    ...
    public function handle()
    {
        ...
        $source_id = 'pad35';
        $source_name = 'StartApp';
        var_dump('StartApp-pad35-'.$dayid);
        $map =[];
        $map['dayid'] = $dayid;
        $map['type'] = 2;
...
//            DataImportImp::saveDataErrorLog(2,$source_id,$source_name,2,$error_msg);
        ...
        foreach ($info as $k => $v) {
        	...
        	foreach ($app_list as $app_k => $app_v) {
                    ...
                    if(($json_info['segId'] == $app_v['platform_app_id'])){
            ...
            if ($num_adtype){
                $error_log_arr['ad_type'][] = isset($json_info['adType']) ? $json_info['adType'].'('.$json_info['prod'].')' : '' ;
            }
        	...
        	if(($num+$num_country+$num_adtype)>0){
                ...
                $error_detail_arr[$k]['first_level_id'] = $json_info['segId'];
                $error_detail_arr[$k]['first_level_name'] = $json_info['prod'];
                $error_detail_arr[$k]['second_level_id'] = '';
                $error_detail_arr[$k]['second_level_name'] = '';
                $error_detail_arr[$k]['money'] = isset($json_info['revenue']) ?number_format($json_info['revenue'] ,6,'.','') : 0.00; // 流水原币
        	...
        	$array[$k]['date'] = $dayid;
            $array[$k]['platform_app_id'] = isset($json_info['segId']) ? addslashes(str_replace('\'\'','\'',$json_info['segId'])) : '';
