...
class UnityHandleProcesses extends Command
{
    ...
    public function handle()
    {
        ...
        self::unityAdDataProcess($source_id,$source_name,$dayid);

    }

    public static function unityAdDataProcess($source_id,$source_name,$dayid){
...
//            DataImportImp::saveDataErrorLog(2,$source_id,'Unity',2,$error_msg);
        ...
        if(!$country_info){
            $error_msg = $source_name.'广告平台数据处理程序国家信息数据查询失败';
            DataImportImp::saveDataErrorLog(2,$source_id,$source_name,2,$error_msg);
            exit;
        }
        ...
        if(!$AdType_info){
            $error_msg = $source_name.'广告平台数据处理程序广告类型数据查询失败';
            DataImportImp::saveDataErrorLog(2,$source_id,$source_name,2,$error_msg);
            exit;
        }
        $array = [];
        ...
        foreach ($info as $k => $v) {
            ...
            if($num){
                ...
                if ($third_app_id && isset($json_info['placement']) && $json_info['placement']){

                    $app_info_sql = "select ad.`id`,ad.`platform_id`,ad.`platform_app_id`,ca.`os_id`,ca.`app_name` from c_app_ad_platform ad left join c_app ca on ad.app_id = ca.id where ad.`platform_id` = '{$source_id}' and ad.status = 1 and ad.`platform_app_id` = '{$third_app_id}' limit 1";
           ...
           if($num_adtype){
            ...
            $array[$k]['ad_unit_name'] = isset($json_info['placement']) ? addslashes(str_replace('\'\'','\'',$json_info['placement'])) : '';
            $array[$k]['date'] = $dayid;
            ...
            $array[$k]['impression'] = isset($json_info['start_count']) ? $json_info['start_count'] : '';
