...
class InmobiHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'InmobiHandleProcesses {dayid?} ';
    ...
    public function handle()
    {
        ...
        if(!$info){
//            $error_msg = $dayid.'号，Inmobi广告平台数据处理程序获取原始数据为空';
//            DataImportImp::saveDataErrorLog(2,'pad02','Inmobi',2,$error_msg);
            exit;
        }
        ...
        if(!$app_list){
        	$error_msg = $dayid.'号，Inmobi广告平台数据处理程序应用数据查询为空';
            DataImportImp::saveDataErrorLog(2,'pad02','Inmobi',2,$error_msg);
            exit;
        }
        ...
        if(!$country_info){
            $error_msg = $dayid.'号，Inmobi广告平台数据处理程序国家信息数据查询为空';
            DataImportImp::saveDataErrorLog(2,'pad02','Inmobi',2,$error_msg);
            exit;
        }
        ...
        foreach ($info as $k => $v) {
        	...
        	foreach ($app_list as $app_k => $app_v) {
        		if($json_info['siteId'] ==$app_v['ad_slot_id'] ){
            ...
            $err_name = (isset($json_info['siteId']) ?$json_info['siteId']:'Null').'#'.(isset($json_info['siteName']) ?$json_info['siteName']:'Null').'#'.(isset($json_info['placementId']) ?$json_info['placementId']:'Null').'#'.(isset($json_info['placementName']) ?$json_info['placementName']:'Null');

            if($num){
                $error_log_arr['siteId'][] = $json_info['siteId'].'('.addslashes(str_replace('\'\'','\'',$err_name)).')';
            }
        	...
        	if(($num+$num_country+$num_adtype)>0){
                ...
                $error_detail_arr[$k]['second_level_id'] = addslashes($json_info['siteId']);
                $error_detail_arr[$k]['second_level_name'] = addslashes(str_replace('\'\'','\'',$json_info['siteName'])) ;
                $error_detail_arr[$k]['money'] = $json_info['earnings'];
        	...
        	$array[$k]['earning'] = $json_info['earnings'];
            ...
            $array[$k]['earning_exc'] = $json_info['earnings']*$divide_ad;
            $array[$k]['earning_flowing'] =isset($json_info['earnings']) ? $json_info['earnings']* $currency_ex: 0;
        	$array[$k]['earning_fix'] = $json_info['earnings']*$currency_ex*$divide_ad;
...
//        	$array[$k]['flow_type'] = '2';//(1,自有流量;2,三方流量)
        ...
        if ($error_log_arr){
            ...
            if(!empty($error_msg_array)) {
                DataImportImp::saveDataErrorLog(2, 'pad02', 'Inmobi', 2, implode(';', $error_msg_array));
            }
