...
class TiktokHandworkHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'TiktokHandworkHandleProcesses {dayid?} ';
    ...
    public function handle()
    {
        ...
        self::TiktokAdDataProcess($channel_list,$source_id,$channel_id,$platform_name,$date_arr,$dayid);
    }


    public static function TiktokAdDataProcess($channel_list,$source_id,$channel_id,$platform_name,$date_arr,$dayid){
        //查询pgsql 的数据
        $map =[];
...
//            DataImportImp::saveDataErrorLog(2,$source_id,$platform_name,2,$error_msg);
        ...
        try {
            foreach ($info as $k => $v) {
                ...
                $err_name = (isset($json_info['广告位ID']) ?$json_info['广告位ID']:'Null').'#'.(isset($json_info['代码位名称']) ?$json_info['代码位名称']:'Null').'#'.(isset($json_info['应用ID']) ?$json_info['应用ID']:'Null').'#'.(isset($json_info['应用名称']) ?$json_info['应用名称']:'Null');
                ...
                if($num){
                    if ($tiktok_app_id && $json_info['代码位ID'] && isset($json_info['代码位类型'])){
                        ...
                        if (isset($app_info_detail[0]) && $app_info_detail[0]){
                            $ad_type = '';
                            if ($json_info['代码位类型'] == '全屏视频'){
                                $ad_type = 1;
                            }elseif ($json_info['代码位类型'] == '激励视频'){
                                $ad_type = 3;
                            }
                            if ($ad_type){
                ...
                if(($num)>0){
                    ...
                    $error_detail_arr[$k]['money'] = $json_info['预估收益(美元)'];
                    $error_detail_arr[$k]['account'] = 'ad-network@zplay.com';
                    unset($array[$k]);
                    //插入错误数据
                    continue;
                }
                ...
                $array[$k]['data_account'] = 'ad-network@zplay.com';
                ...
                $array[$k]['earning'] = $json_info['预估收益(美元)'];
        ...
        and b.platform_id = a.platform_id and a.`customer_id` = b.`customer_id` and a.`company_id` = b.`company_id` and b.`currency_type_id` = c.`currency_id` and c.effective_time = '{$effective_time}' ";
                ...
                $array[$k]['earning_exc'] = $json_info['预估收益(美元)']*$divide_ad;
                $array[$k]['earning_flowing'] =isset($json_info['预估收益(美元)']) ? $json_info['预估收益(美元)']*$currency_ex: 0;
                $array[$k]['earning_fix'] = $json_info['预估收益(美元)']*$divide_ad*$currency_ex;
