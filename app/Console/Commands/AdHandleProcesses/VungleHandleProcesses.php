...
class VungleHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'VungleHandleProcesses {dayid?} ';
    ...
    public static function VungleAdDataProcess($source_id,$source_name,$dayid){
        ...
        if(!$info){
//            $error_msg = $dayid.'号，Vungle广告平台数据处理程序获取原始数据为空';
//            DataImportImp::saveDataErrorLog(2,$source_id,'Vungle',2,$error_msg);
            exit;
        }
        ...
        if(!$app_list){
        	$error_msg = 'Vungle广告平台数据处理程序应用数据查询为空';
            DataImportImp::saveDataErrorLog(2,$source_id,'Vungle',2,$error_msg);
            exit;
        }
        ...
        if(!$country_info){
            $error_msg = 'Vungle广告平台数据处理程序国家信息数据查询为空';
            DataImportImp::saveDataErrorLog(2,$source_id,'Vungle',2,$error_msg);
            exit;
        }
        ...
        foreach ($info as $k => $v) {
        	...
        	foreach ($app_list as $app_k => $app_v) {
        		if($json_info['application id'] ==$app_v['platform_app_id'] &&  $json_info['placement reference id'] ==$app_v['ad_slot_id'] ){
        	...
        	if(($num+$num_country+$num_adtype)>0){
                ...
                $error_detail_arr[$k]['first_level_id'] = $json_info['application id'];
                $error_detail_arr[$k]['first_level_name'] = addslashes(str_replace('\'\'','\'',$json_info['application name']));
                $error_detail_arr[$k]['second_level_id'] = $json_info['placement reference id'];
                $error_detail_arr[$k]['second_level_name'] = addslashes(str_replace('\'\'','\'',$json_info['placement name']));
        	...
        	$array[$k]['platform_app_id'] = addslashes($json_info['application id']);
        	$array[$k]['platform_app_name'] = addslashes(str_replace('\'\'','\'',$json_info['application name']));
            $array[$k]['ad_unit_id'] = addslashes($json_info['placement reference id']);
        	$array[$k]['ad_unit_name'] = addslashes(str_replace('\'\'','\'',$json_info['placement name']));
        	$array[$k]['impression'] = $json_info['views'];
        	$array[$k]['all_request'] = $json_info['completes'];
        	$array[$k]['success_requests'] = $json_info['completes'];
