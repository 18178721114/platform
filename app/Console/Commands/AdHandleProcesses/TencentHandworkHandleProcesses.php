...
class TencentHandworkHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'TencentHandworkHandleProcesses {dayid?} ';
    ...
    public function handle()
    {
        ...
        if(!$AdType_info){
            echo "广告类型数据查询失败";
            $error_msg = $dayid.'号，'.$platform_name.'广告平台数据处理程序广告类型数据查询为空';
            DataImportImp::saveDataErrorLog(2,$source_id,$platform_name,2,$error_msg);
            exit;
        }
        ...
        try {
            foreach ($info as $k => $v) {
            	...
            	if(($num)>0){                
                    ...
                    $error_detail_arr[$k]['first_level_name'] = addslashes(str_replace('\'\'','\'',$json_info['媒体名称']));
                    $error_detail_arr[$k]['second_level_id'] = '';
                    $error_detail_arr[$k]['second_level_name'] = addslashes(str_replace('\'\'','\'',$json_info['广告位名称']));
                    $error_detail_arr[$k]['money'] = $json_info['预计收入'];
                ...
                $array[$k]['platform_app_name'] = addslashes(str_replace('\'\'','\'',$json_info['媒体名称']));
            	$array[$k]['ad_unit_name'] = addslashes(str_replace('\'\'','\'',$json_info['广告位名称']));
            	$array[$k]['impression'] = $json_info['广告展示次数'];
            	$array[$k]['click'] = $json_info['点击量'];
            	$array[$k]['earning'] = $json_info['预计收入'];
                if($app_list[0]['divide_ad']){
                    $divide_ad = 1-$app_list[0]['divide_ad']/100;
                }else{
                    $divide_ad =1;
                }
            	$array[$k]['earning_exc'] = $json_info['预计收入']*$divide_ad;
                $array[$k]['earning_flowing'] = $json_info['预计收入'];
            	$array[$k]['earning_fix'] = $json_info['预计收入']*$divide_ad;
