...
class AdmobHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'AdmobHandleProcesses {dayid?} {data_account?}';
...
//            DataImportImp::saveDataErrorLog(2,$source_id,$source_name,2,$error_msg);
        ...
        foreach ($info as $k => $v) {
        	...
        	foreach ($app_list as $app_k => $app_v) {
        		if(isset($json_info['ad_unit_id']) && ($json_info['ad_unit_id'] == $app_v['ad_slot_id'])){
        	...
        	if(($num+$num_country)>0){
                ...
                $error_detail_arr[$k]['second_level_id'] = $json_info['ad_unit_id'];
                $error_detail_arr[$k]['second_level_name'] = $json_info['ad_unit_name'];
                $error_detail_arr[$k]['money'] = isset($json_info['earnings']) ? $json_info['earnings'] : 0.00; // 流水原币
                $error_detail_arr[$k]['account'] = $json_info['account'];
        	...
        	$array[$k]['ad_unit_name'] = isset($json_info['ad_unit_name']) ? addslashes(str_replace('\'\'','\'',$json_info['ad_unit_name'])) : '';
        	$array[$k]['ad_unit_id'] = isset($json_info['ad_unit_id']) ? $json_info['ad_unit_id'] : '';
        	$array[$k]['success_requests'] = isset($json_info['ad_requests']) ? $json_info['ad_requests'] : 0;
        	$array[$k]['all_request'] = $array[$k]['success_requests'];
        	$array[$k]['impression'] = isset($json_info['individual_ad_impressions']) ? $json_info['individual_ad_impressions'] : 0;
        	$array[$k]['click'] = isset($json_info['clicks']) ? $json_info['clicks'] : 0;

        	$array[$k]['earning'] = isset($json_info['earnings']) ? $json_info['earnings'] : 0.00; // 流水原币
            ...
            $array[$k]['earning_exc'] = isset($json_info['earnings']) ? $json_info['earnings'] * $divide_ad: 0;
            ...
            //收入流水人民币
            $array[$k]['earning_flowing'] =isset($json_info['earnings']) ? $json_info['earnings']* $currency_ex: 0;
            $array[$k]['earning_fix'] = isset($json_info['earnings']) ? $json_info['earnings'] * $currency_ex * $divide_ad : 0;
        	...
        	$array[$k]['statistics'] = '0'; // 0是三方统计 2是玉米统计
