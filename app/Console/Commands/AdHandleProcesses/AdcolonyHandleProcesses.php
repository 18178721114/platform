...
class AdcolonyHandleProcesses extends Command
{
     ...
     * @var string
     */
    protected $signature = 'AdcolonyHandleProcesses {dayid?} ';
...
//            DataImportImp::saveDataErrorLog(2,$source_id,$source_name,2,$error_msg);
        ...
        foreach ($info as $k => $v) {
        	...
        	foreach ($app_list as $app_k => $app_v) {
                    ...
                    if(($json_info['zone_id'] == $app_v['ad_slot_id'])){
        	...
        	if(($num+$num_country+$num_adtype)>0){
                ...
                $error_detail_arr[$k]['second_level_id'] = $json_info['zone_id'];
                $error_detail_arr[$k]['second_level_name'] = $json_info['zone_name'];
                $error_detail_arr[$k]['money'] = isset($json_info['earnings']) ?number_format($json_info['earnings'] ,6,'.','') : 0.00; // 流水原币
            ...
            $array[$k]['ad_unit_id'] = isset($json_info['zone_id']) ? addslashes(str_replace('\'\'','\'',$json_info['zone_id'])) : '';
            $array[$k]['ad_unit_name'] = isset($json_info['zone_name']) ? addslashes(str_replace('\'\'','\'',$json_info['zone_name'])) : '';
        	$array[$k]['impression'] = $json_info['impressions'];
        	$array[$k]['click'] = $json_info['clicks'];

            $array[$k]['earning'] = isset($json_info['earnings']) ?number_format($json_info['earnings'] ,6,'.','') : 0.00; // 流水原币
            ...
            $array[$k]['earning_exc'] = isset($json_info['earnings']) ? number_format($json_info['earnings'],6,'.','') * $divide_ad: 0;
            ...
            $array[$k]['earning_flowing'] =isset($json_info['revenue_usd']) ? number_format($json_info['earnings'],6,'.','') * $currency_ex: 0;
            $array[$k]['earning_fix'] = isset($json_info['revenue_usd']) ? number_format($json_info['earnings'],6,'.','') * $currency_ex * $divide_ad : 0;
