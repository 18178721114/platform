...
class JuliangAdiHandworkHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'JuliangAdiHandworkHandleProcesses {dayid?} ';
        ...
        try {
            foreach ($info as $k => $v) {
            	...
            	foreach ($app_list as $app_k => $app_v) {
            		...
            		if($json_info['应用ID'] == $app_v['platform_app_id'] ){
...
//                }
            	...
            	if(($num+$num_adtype)>0){
                    ...
                    $error_detail_arr[$k]['first_level_name'] = addslashes(str_replace('\'\'','\'',$json_info['游戏名称']));
                    $error_detail_arr[$k]['second_level_id'] = '';
                    $error_detail_arr[$k]['second_level_name'] = '';
                    $error_detail_arr[$k]['money'] = $json_info['广告收入'];
...
//            	$array[$k]['click'] = $json_info['点击量'];
            	$array[$k]['earning'] = $json_info['广告收入'];
                if($app_list[0]['divide_ad']){
                    $divide_ad = 1-$app_list[0]['divide_ad']/100;
                }else{
                    $divide_ad =1;
                }
                $array[$k]['earning_exc'] = $json_info['广告收入']*$divide_ad;
                $array[$k]['earning_flowing'] =isset($json_info['广告收入']) ? $json_info['广告收入']: 0;
            	$array[$k]['earning_fix'] = $json_info['广告收入']*$divide_ad;
