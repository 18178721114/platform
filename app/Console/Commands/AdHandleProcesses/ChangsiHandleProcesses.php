...
class ChangsiHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ChangsiHandleProcesses {dayid?} ';
...
//            DataImportImp::saveDataErrorLog(2,$source_id,$source_name,2,$error_msg);
        ...
        foreach ($info as $k => $v) {
        	...
        	foreach ($app_list as $app_k => $app_v) {
        		if(addslashes(str_replace('\'\'','\'',$json_info['appname'])) == $app_v['platform_app_name']){
            ...
            foreach ($AdType_info as $AdType_k => $AdType_v) {
                if($json_info['adform'] == $AdType_v['name'] ){
        	...
        	if( $num+$num_adtype > 0){
                ...
                $error_detail_arr[$k]['first_level_id'] = addslashes(str_replace('\'\'','\'',$json_info['appname']));
                ...
                $error_detail_arr[$k]['money'] = isset($json_info['income']) ? $json_info['income'] : 0.00; // 流水原币
