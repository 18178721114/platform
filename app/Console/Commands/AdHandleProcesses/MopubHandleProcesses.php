...
class MopubHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'MopubHandleProcesses {dayid?} ';
...
//            DataImportImp::saveDataErrorLog(2,$source_id,$source_name,2,$error_msg);
        ...
        foreach ($info as $k => $v) {
        	...
        	if(isset($json_info['order']) ) {
                if ($json_info['order'] != 'Network' or $json_info['order'] != 'Marketplace' or $json_info['order'] != 'iAd Global Segment') {
                    continue;
                }
            }
                ...
                $err_name = (isset($json_info['adunit_id']) ?$json_info['adunit_id']:'Null').'#'.(isset($json_info['adunit']) ?$json_info['adunit']:'Null').'#'.(isset($json_info['app_id']) ?$json_info['app_id']:'Null').'#'.(isset($json_info['app']) ?$json_info['app']:'Null');

                if ($num){
                    $error_log_arr['application'][] = $json_info['app_id'].'('.$err_name.')';
                }
                ...
                foreach ($AdType_info as $AdType_k => $AdType_v) {
                    if($json_info['adunit_format'] == $AdType_v['name'] ){
                ...
                if ($num_adtype){
                    $error_log_arr['ad_type'][] = isset($json_info['adunit_format']) ? $json_info['adunit_format'].'('.$err_name.')' : '' ;
                }
                ...
                if(($num+$num_country+$num_adtype)>0){
                    ...
                    $err_app_id = isset($json_info['app_id']) ? addslashes(str_replace('\'\'','\'',$json_info['app_id'])) : '';
                    $err_app = isset($json_info['app']) ? addslashes(str_replace('\'\'','\'',$json_info['app'])) : '';
                    $err_adunit_id =  isset($json_info['adunit_id']) ? addslashes(str_replace('\'\'','\'',$json_info['adunit_id'])) : '';
                    $err_adunit = isset($json_info['adunit']) ? addslashes(str_replace('\'\'','\'',$json_info['adunit'])) : '';

                    $error_detail_arr[$k]['first_level_id'] = $err_app_id;
                    $error_detail_arr[$k]['first_level_name'] = $err_app;
                    $error_detail_arr[$k]['second_level_id'] = ''; // $err_adunit_id;
                    $error_detail_arr[$k]['second_level_name'] = $err_adunit;
                ...
                $array[$k]['date'] = $dayid;
                $array[$k]['platform_app_id'] = isset($json_info['app_id']) ? addslashes(str_replace('\'\'','\'',$json_info['app_id'])) : '';
                $array[$k]['platform_app_name'] = isset($json_info['app']) ? addslashes(str_replace('\'\'','\'',$json_info['app'])) : '';
                $array[$k]['ad_unit_id'] = isset($json_info['adunit_id']) ? addslashes(str_replace('\'\'','\'',$json_info['adunit_id'])) : '';
                $array[$k]['ad_unit_name'] = isset($json_info['adunit']) ? addslashes(str_replace('\'\'','\'',$json_info['adunit'])) : '';
                $array[$k]['impression'] = $json_info['impressions'];
                $array[$k]['click'] = $json_info['clicks'];

                $array[$k]['earning'] = isset($json_info['revenue']) ?number_format($json_info['revenue'] ,6,'.','') : 0.00; // 流水原币
                ...
                $array[$k]['earning_exc'] = isset($json_info['revenue']) ? number_format($json_info['revenue'],6,'.','') * $divide_ad: 0;
                ...
                $array[$k]['earning_flowing'] =isset($json_info['revenue_usd']) ? number_format($json_info['revenue'],6,'.','') * $currency_ex: 0;
                $array[$k]['earning_fix'] = isset($json_info['revenue_usd']) ? number_format($json_info['revenue'],6,'.','') * $currency_ex * $divide_ad : 0;
    ...
    //        	$array[$k]['flow_type'] = '2';//(1,自有流量;2,三方流量)
        ...
        if ($error_log_arr){
            ...
            if (isset($error_log_arr['application']) && !empty($error_log_arr['application'])){
                ...
                $error_msg_array[] = '应用id匹配失败,ID为:'.$application;
                $error_msg_mail[] = '应用id匹配失败，ID为：'.$application;
            }
