...
class XiaomiHandworkHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'XiaomiHandworkHandleProcesses {dayid?} ';
...
//            DataImportImp::saveDataErrorLog(2,$source_id,$platform_name,2,$error_msg);
        ...
        if(!$app_list){
            $error_msg = $platform_name.'渠道广告数据处理程序应用数据查询为空';
            DataImportImp::saveDataErrorLog(2,$source_id,$platform_name,2,$error_msg);
        ...
        try {
            foreach ($info as $k => $v) {
            	...
            	foreach ($app_list as $app_k => $app_v) {
            		...
            		if($json_info['广告位ID'] == $app_v['ad_slot_id'] ){
                ...
                $err_name = (isset($json_info['广告位ID']) ?$json_info['广告位ID']:'Null').'#'.(isset($json_info['广告位名称']) ?$json_info['广告位名称']:'Null').'#'.(isset($json_info['应用ID']) ?$json_info['应用ID']:'Null').'#'.(isset($json_info['游戏名称']) ?$json_info['游戏名称']:'Null');
            	...
            	if(($num)>0){
                    ...
                    $error_detail_arr[$k]['second_level_id'] = addslashes($json_info['广告位ID']);
                    $error_detail_arr[$k]['second_level_name'] = addslashes(str_replace('\'\'','\'',$json_info['广告位名称']));
                    $error_detail_arr[$k]['money'] = $json_info['预计收入(元)'];
            	...
            	$array[$k]['ad_unit_name'] = addslashes(str_replace('\'\'','\'',$json_info['广告位名称']));
            	$array[$k]['impression'] = $json_info['展现量'];
            	$array[$k]['click'] = $json_info['点击数'];
            	$array[$k]['earning'] = $json_info['预计收入(元)'];
                if($app_list[0]['divide_ad']){
                    $divide_ad = 1-$app_list[0]['divide_ad']/100;
                }else{
                    $divide_ad =1;
                }
                $array[$k]['earning_exc'] = $json_info['预计收入(元)']*$divide_ad;
                $array[$k]['earning_flowing'] =isset($json_info['预计收入(元)']) ? $json_info['预计收入(元)']: 0;
            	$array[$k]['earning_fix'] = $json_info['预计收入(元)']*$divide_ad;
