...
class MobvistaHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'MobvistaHandleProcesses {dayid?} ';
    ...
    public function handle()
    {
        ...
        if(!$info){
//            $error_msg = $dayid.'号，Mobvista广告平台数据处理程序获取原始数据为空';
//            DataImportImp::saveDataErrorLog(2,$source_id,'Mobvista',2,$error_msg);
            exit;
        }
        ...
        if(!$app_list){
        	$error_msg = $dayid.'号，Mobvista广告平台数据处理程序应用数据查询为空';
            DataImportImp::saveDataErrorLog(2,$source_id,'Mobvista',2,$error_msg);
            exit;
        }
        ...
        if(!$country_info){
            $error_msg = $dayid.'号，Mobvista广告平台数据处理程序国家信息数据查询为空';
            DataImportImp::saveDataErrorLog(2,$source_id,'Mobvista',2,$error_msg);
            exit;
        }
        ...
        foreach ($info as $k => $v) {
        	...
        	foreach ($app_list as $app_k => $app_v) {
        		if($json_info['unit_id'] ==$app_v['ad_slot_id'] ){
            ...
            if($num){
                $error_log_arr['app_id'][] = $json_info['unit_id'].'('.addslashes(str_replace('\'\'','\'',$json_info['unit_name'])).')';
            }
        	...
        	if(($num+$num_country+$num_adtype)>0){
                ...
                $err_app_id = addslashes($json_info['app_id']);
                $err_app_name = addslashes(str_replace('\'\'','\'',$json_info['app_name']));
                $err_unit_id = addslashes($json_info['unit_id']);
                $err_unit_name = addslashes(str_replace('\'\'','\'',$json_info['unit_name']));

                $error_detail_arr[$k]['first_level_id'] = ''; // $err_app_id;
                $error_detail_arr[$k]['first_level_name'] = $err_app_name;
                $error_detail_arr[$k]['second_level_id'] = $err_unit_id;
                $error_detail_arr[$k]['second_level_name'] = $err_unit_name;
                $error_detail_arr[$k]['money'] = $json_info['est_revenue'];
            ...
            $array[$k]['ad_unit_id'] = addslashes($json_info['unit_id']);
        	$array[$k]['ad_unit_name'] = addslashes(str_replace('\'\'','\'',$json_info['unit_name']));
        	$array[$k]['impression'] = $json_info['impression'];
        	$array[$k]['all_request'] = $json_info['request'];
        	$array[$k]['click'] = $json_info['click'];
        	$array[$k]['earning'] = $json_info['est_revenue'];
        	...
        	$array[$k]['earning_exc'] = $json_info['est_revenue']*$divide_ad;
            $array[$k]['earning_flowing'] =isset($json_info['est_revenue']) ? $json_info['est_revenue']* $currency_ex: 0;
        	$array[$k]['earning_fix'] = $json_info['est_revenue']*$currency_ex*$divide_ad;
...
//        	$array[$k]['flow_type'] = '2';//(1,自有流量;2,三方流量)
        ...
        if ($error_log_arr){
            ...
            DataImportImp::saveDataErrorLog(2,$source_id,'Mobvista',2,implode(';',$error_msg_array));
            DataImportImp::saveDataErrorMoneyLog($source_id,$dayid,$error_detail_arr);

            //CommonFunction::sendMail($error_msg_mail,'Mobvista广告平台数据处理error');
        }
        if(!empty($array)){
