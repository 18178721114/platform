...
class a4399HandworkHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'a4399HandworkHandleProcesses {dayid?} ';
    ...
    public function handle()
    {
        ...
        if(!$info){
//        	 $error_msg = $dayid.'号,'.$platform_name.'渠道数据处理程序获取原始数据为空';
//            DataImportImp::saveDataErrorLog(2,$source_id,$platform_name,2,$error_msg);
        ...
        if(!$app_list){
            $error_msg = $dayid.'号,'.$platform_name.'渠道数据处理程序应用数据查询为空';
            DataImportImp::saveDataErrorLog(2,$source_id,$platform_name,2,$error_msg);
        ...
        try {
            foreach ($info as $k => $v) {
            	...
            	foreach ($app_list as $app_k => $app_v) {
            		...
            		if($json_info['应用ID'] == $app_v['platform_app_id']  &&  $json_info['广告位ID'] == $app_v['ad_slot_id'] ){
            	...
            	if(($num)>0){                
                    ...
                    $error_detail_arr[$k]['first_level_id'] = addslashes(str_replace('\'\'','\'',$json_info['应用ID']));
                    $error_detail_arr[$k]['first_level_name'] = addslashes(str_replace('\'\'','\'',$json_info['应用']));
                    $error_detail_arr[$k]['second_level_id'] = addslashes(str_replace('\'\'','\'',$json_info['广告位ID']));
                    $error_detail_arr[$k]['second_level_name'] = addslashes(str_replace('\'\'','\'',$json_info['广告位']));
                ...
                $array[$k]['earning_flowing'] = $json_info['预计收入（元）'];
