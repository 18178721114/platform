...
class HuaweiHandworkHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'HuaweiHandworkHandleProcesses {dayid?} ';
        ...
        try {
            foreach ($info as $k => $v) {
            	...
            	if(($num)>0){
                    ...
                    $error_detail_arr[$k]['money'] = $json_info['预估总收益（元）'];
                ...
                $array[$k]['platform_app_name'] = $json_info['应用名称'];
            	$array[$k]['ad_unit_id'] = '';
            	$array[$k]['ad_unit_name'] = '';
            	$array[$k]['impression'] = $json_info['展示量'];
            	$array[$k]['click'] = $json_info['点击量'];
            	$array[$k]['earning'] = $json_info['预估总收益（元）'];
                if($app_list[0]['divide_ad']){
                    $divide_ad = 1-$app_list[0]['divide_ad']/100;
                }else{
                    $divide_ad =1;
                }
            	$array[$k]['earning_exc'] = $json_info['预估总收益（元）']*$divide_ad;
                $array[$k]['earning_flowing'] =isset($json_info['预估总收益（元）']) ? $json_info['预估总收益（元）']: 0;
            	$array[$k]['earning_fix'] = $json_info['预估总收益（元）']*$divide_ad;
    ...
    //        	$array[$k]['flow_type'] = '2';//(1,自有流量;2,三方流量)
        ...
        if ($error_log_arr){
            ...
            if (isset($error_log_arr['app_id'])){
                ...
                $error_msg_mail[] = '应用名称匹配失败，ID为：'.$app_id;
            }
