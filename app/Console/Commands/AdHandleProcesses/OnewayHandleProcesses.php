...
class OnewayHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'OnewayHandleProcesses {dayid?} ';
    ...
    public function handle()
    {
        ...
        $source_id = 'pad52';
        $source_name = 'OneWay';
        var_dump('OneWay-pad52-'.$dayid);
        try{
            self::OnewayAdDataProcess($source_id, $source_name, $dayid);
        }catch (\Exception $e) {
    ...
    public static function OnewayAdDataProcess($source_id,$source_name,$dayid){
   ...
   $sql = "SELECT DISTINCT
        ...
        foreach ($info as $k => $v) {
        	...
        	foreach ($app_list as $app_k => $app_v) {
        		if(($json_info['publishId'] == $app_v['platform_app_id']) && ($json_info['placementId'] == $app_v['ad_slot_id'])){
            ...
            $err_name = (isset($json_info['placementId']) ?$json_info['placementId']:'Null').'#Null#'.(isset($json_info['publishId']) ?$json_info['publishId']:'Null').'#'.(isset($json_info['appName']) ?$json_info['appName']:'Null');

            if ($num){
                if($json_info['publishId'] && $json_info['placementId']){
                    $app_info_sql = "select ad.`id`,ad.`platform_id`,ad.`platform_app_id`,ca.`os_id`,ca.`app_name`
                                     ...
                                     and ad.`platform_app_id` = '{$json_info['publishId']}' limit 1";
                ...
                $error_log_arr['app_id'][] = $json_info['publishId'].'或'.$json_info['placementId'].'('.addslashes(str_replace('\'\'','\'',$err_name)).')';
            }
        	...
        	if(($num + $num_country) > 0){
                ...
                $error_detail_arr[$k]['first_level_id'] = $json_info['publishId'];
                $error_detail_arr[$k]['first_level_name'] = addslashes(str_replace('\'\'','\'',$json_info['appName']));
                $error_detail_arr[$k]['second_level_id'] = $json_info['placementId'];
        	...
        	$array[$k]['date'] = $dayid;
        	$array[$k]['platform_app_id'] = $json_info['publishId'];
        	$array[$k]['platform_app_name'] = addslashes(str_replace('\'\'','\'',$json_info['appName']));
        	$array[$k]['ad_unit_id'] = $json_info['placementId'];

        	$array[$k]['impression'] = isset($json_info['views']) ? $json_info['views'] : 0;
