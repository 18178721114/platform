...
class KewanHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'KewanHandleProcesses {dayid?} ';
    ...
    public function handle()
    {
        ...
        if(!$info){
//            $error_msg = $dayid.'号，KeWan广告平台数据处理程序获取原始数据为空';
//            DataImportImp::saveDataErrorLog(2,$source_id,'KeWan',2,$error_msg);
            exit;
        }
        ...
        if(!$country_info){
            $error_msg = $dayid.'号，KeWan广告平台数据处理程序国家信息数据查询为空';
            DataImportImp::saveDataErrorLog(2,$source_id,'KeWan',2,$error_msg);
            exit;
        }
        ...
        foreach ($info as $k => $v) {
        	...
        	foreach ($app_list as $app_k => $app_v) {
        		if($json_info['ad_unit_id'] ==$app_v['ad_slot_id'] ){
            ...
            $err_name = (isset($json_info['ad_unit_id']) ?$json_info['ad_unit_id']:'Null').'#Null#'.(isset($json_info['app_id']) ?$json_info['app_id']:'Null').'#'.(isset($json_info['name']) ?$json_info['name']:'Null');

            if($num){
                $error_log_arr['app_id'][] = $json_info['ad_unit_id'].'('.addslashes(str_replace('\'\'','\'',$err_name)).')';
            }
        	...
        	if(($num+$num_country+$num_adtype)>0){
                ...
                $error_detail_arr[$k]['first_level_name'] = addslashes(str_replace('\'\'','\'',$json_info['name']));
                $error_detail_arr[$k]['second_level_id'] = addslashes($json_info['ad_unit_id']);
                $error_detail_arr[$k]['second_level_name'] = '';
                $error_detail_arr[$k]['money'] = $json_info['income']/100;
        	...
        	$array[$k]['platform_app_name'] = addslashes(str_replace('\'\'','\'',$json_info['name']));
            $array[$k]['ad_unit_id'] = addslashes($json_info['ad_unit_id']);
        	$array[$k]['impression'] = $json_info['imp'];
        	$array[$k]['all_request'] = $json_info['request'];
        	$array[$k]['click'] = $json_info['click'];
        	$array[$k]['earning'] = $json_info['income']/100;
        	...
        	$array[$k]['earning_exc'] = $json_info['income']*$divide_ad/100;
            $array[$k]['earning_flowing'] =isset($json_info['income']) ? $json_info['income']/100* $currency_ex: 0;
        	$array[$k]['earning_fix'] = $json_info['income']*$currency_ex*$divide_ad/100;
...
//        	$array[$k]['flow_type'] = '2';//(1,自有流量;2,三方流量)
        ...
        if ($error_log_arr){
            ...
            if(!empty($error_msg_array)) {
                DataImportImp::saveDataErrorLog(2, $source_id, 'KeWan', 2, implode(';', $error_msg_array));
            }
            DataImportImp::saveDataErrorMoneyLog($source_id,$dayid,$error_detail_arr);

            //CommonFunction::sendMail($error_msg_mail,'KeWan广告平台数据处理error');
        }
        if(!empty($array)){
