...
class SmaatoHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'SmaatoHandleProcesses {dayid?} ';
...
//            DataImportImp::saveDataErrorLog(2,$source_id,$source_name,2,$error_msg);
        ...
        foreach ($info as $k => $v) {
        	...
        	foreach ($app_list as $app_k => $app_v) {
        		if(isset($json_info['AdspaceId']) && ($json_info['AdspaceId'] == $app_v['ad_slot_id'])){
            ...
            $err_name = (isset($json_info['AdspaceId']) ?$json_info['AdspaceId']:'Null').'#Null#'.(isset($json_info['appid']) ?$json_info['appid']:'Null').'#'.(isset($json_info['appname']) ?$json_info['appname']:'Null');


            if ($num){
                $error_log_arr['ad_slot_id'][] = $json_info['AdspaceId'].'('.addslashes(str_replace('\'\'','\'',$err_name)).')';
            }
            ...
            if(($num+$num_country)>0){
                ...
                $error_detail_arr[$k]['second_level_id'] = $json_info['AdspaceId'];
                $error_detail_arr[$k]['second_level_name'] = '';
                $error_detail_arr[$k]['money'] = $json_info['grossRevenue'];
        	...
        	$array[$k]['date'] = $dayid;
        	$array[$k]['platform_app_id'] = $json_info['appid'];
            $array[$k]['platform_app_name'] = addslashes(str_replace('\'\'','\'',$json_info['appname']));
            $array[$k]['ad_unit_id'] = $json_info['AdspaceId'];

        	$array[$k]['impression'] = $json_info['impressions'];
        	$array[$k]['click'] = $json_info['clicks'];

            $array[$k]['earning'] = isset($json_info['grossRevenue']) ? $json_info['grossRevenue'] : 0.00; // 流水原币
            ...
            $array[$k]['earning_exc'] = isset($json_info['grossRevenue']) ? $json_info['grossRevenue'] * $divide_ad: 0;
            ...
            $array[$k]['earning_flowing'] =isset($json_info['grossRevenue']) ? $json_info['grossRevenue']* $currency_ex: 0;
            $array[$k]['earning_fix'] = isset($json_info['grossRevenue']) ? $json_info['grossRevenue'] * $currency_ex * $divide_ad : 0;
...
//        	$array[$k]['flow_type'] = '2';//(1,自有流量;2,三方流量)
        ...
        if ($error_log_arr){
            ...
            if (isset($error_log_arr['ad_slot_id']) && !empty($error_log_arr['ad_slot_id'])){
                $ad_slot_id = implode(',',array_unique($error_log_arr['ad_slot_id']));
                $error_msg_array[] = '广告位匹配失败,ID为:'.$ad_slot_id;
                $error_msg_mail[] = '广告位匹配失败，ID为：'.$ad_slot_id;
            }
