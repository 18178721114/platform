...
class FyberHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'FyberHandleProcesses {dayid?} ';
...
//            DataImportImp::saveDataErrorLog(2,$source_id,$source_name,2,$error_msg);
        ...
        foreach ($info as $k => $v) {
            ...
            if ($json_info['ad_network'] == 'FyberExchange' && $v['account'] == 'noodlecake') {


                foreach ($app_list as $app_k => $app_v) {

                    if (($json_info['application_id'] == $app_v['platform_app_id'])) {
                ...
                $err_name = 'Null#Null#'.(isset($json_info['application_id']) ?$json_info['application_id']:'Null').'#'.(isset($json_info['application_name']) ?$json_info['application_name']:'Null');

                if ($num) {
                    $error_log_arr['application'][] = $json_info['application_id'].'('.$err_name.')';
                }
                ...
                if (($num + $num_country + $num_adtype) > 0) {
                    ...
                    $error_detail_arr[$k]['first_level_id'] = isset($json_info['application_id']) ? addslashes(str_replace('\'\'','\'',$json_info['application_id'])) : '';
                    $error_detail_arr[$k]['first_level_name'] = isset($json_info['application_name']) ? addslashes(str_replace('\'\'','\'',$json_info['application_name'])) : '';
                    $error_detail_arr[$k]['second_level_id'] = '';
                    $error_detail_arr[$k]['second_level_name'] = '';
                    $error_detail_arr[$k]['money'] = $json_info['revenue_usd'];
                ...
                $array[$k]['date'] = $dayid;
                $array[$k]['platform_app_id'] = isset($json_info['application_id']) ? addslashes(str_replace('\'\'', '\'', $json_info['application_id'])) : '';
                $array[$k]['platform_app_name'] = isset($json_info['application_name']) ? addslashes(str_replace('\'\'', '\'', $json_info['application_name'])) : '';
                $array[$k]['impression'] = $json_info['impressions'];
                $array[$k]['success_requests'] = $json_info['requests'];
                $array[$k]['all_request'] = $json_info['requests'];

                $array[$k]['earning'] = isset($json_info['revenue_usd']) ? $json_info['revenue_usd'] : 0.00; // 流水原币
                ...
                $array[$k]['earning_exc'] = isset($json_info['revenue_usd']) ? $json_info['revenue_usd'] * $divide_ad : 0;
                ...
                $array[$k]['earning_flowing'] = isset($json_info['revenue_usd']) ? $json_info['revenue_usd'] * $currency_ex : 0;
                $array[$k]['earning_fix'] = isset($json_info['revenue_usd']) ? $json_info['revenue_usd'] * $currency_ex * $divide_ad : 0;
...
//        	$array[$k]['flow_type'] = '2';//(1,自有流量;2,三方流量)
        ...
        if ($error_log_arr) {
            ...
            if (isset($error_log_arr['country']) && !empty($error_log_arr['country'])) {
                $ad_type = implode(',', array_unique($error_log_arr['country']));
                $error_msg_array[] = '国家匹配失败,code为:' . $ad_type;
                $error_msg_mail[] = '国家匹配失败，code为：' . $ad_type;
            }
...
//                CommonFunction::sendMail($error_msg_mail, $source_name . '广告平台数据处理error');
        ...
        // 保存正确数据
        if ($array) {
            Redis::hset('zplay.test.ad.plat.info', $source_id, $dayid);
            //拆分批次
            $step = array();
            $i = 0;
            ...
            if ($step) {
                foreach ($step as $k => $v_info) {
                    ...
                    Redis::rpush('zplay.test.ad.info', $sql_str);
                }
            }
