...
class HeyzapHandleProcesses extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'HeyzapHandleProcesses {dayid?} ';
    ...
    public function handle()
    {
        ...
        $map[] =['income','<>',0] ;
        $map['like'][] = ["json_data->network",'like','heyzap'];
...
//            DataImportImp::saveDataErrorLog(2,$source_id,$source_name,2,$error_msg);
        ...
        foreach ($info as $k => $v) {
        	...
        	if(($num+$num_country+$num_adtype)>0){
                ...
                $error_detail_arr[$k]['first_level_id'] = isset($json_info['app_id']) ? addslashes($json_info['app_id']) : '';
                $error_detail_arr[$k]['first_level_name'] = isset($json_info['app_name']) ? addslashes(str_replace('\'\'','\'',$json_info['app_name'])) : '';
                $error_detail_arr[$k]['second_level_id'] ='';
                $error_detail_arr[$k]['second_level_name'] ='';
                $error_detail_arr[$k]['money'] = isset($json_info['revenue_in_cents']) ? $json_info['revenue_in_cents'] / 100 : 0.00; // 流水原币
            ...
            // {"country_code":"TW","currency":"USD","revenue_in_cents":0,"impressions":9,"clicks":0,"ecpm_cents":0,"app_id":"1305551","app_name":"The Blockheads","network":"vungle","platform":"android","format":"rewarded"}
            ...
            $array[$k]['earning'] = isset($json_info['revenue_in_cents']) ? $json_info['revenue_in_cents'] / 100 : 0.00; // 流水原币
            ...
            $array[$k]['earning_exc'] = isset($json_info['revenue_in_cents']) ? $json_info['revenue_in_cents'] * $divide_ad  / 100 : 0;
            ...
            $array[$k]['earning_flowing'] =isset($json_info['revenue_in_cents']) ? $json_info['revenue_in_cents']* $currency_ex/100: 0;
            $array[$k]['earning_fix'] = isset($json_info['revenue_in_cents']) ? $json_info['revenue_in_cents'] * $currency_ex * $divide_ad  / 100 : 0;
