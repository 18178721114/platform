    ...
    public function handle()
    {
        ...
        foreach ($info as $key => $value) {
            ...
            if(!empty($appInfoList['data'])){
                    ...
                    if(!empty($dataInfo['data'])){
                        foreach ($dataInfo['data'] as $ad_k => $ad_v) {
                            ...
                            if (isset($ad_stats_data['error']) && $ad_stats_data['error'] == '' && isset($ad_stats_data['data']) ) {
                                ...
                                $error_msg = AD_PLATFORM.'推广平台'.$value['company_username'].'账号下应用'.$appInfo['app_id'].'广告位'.$ad_v['ad_id'].'数据为空,错误信息'.$ad_stats_data['error'];
                        ...
                        $error_msg = AD_PLATFORM.'推广平台'.$value['company_username'].'账号下应用'.$appInfo['app_id'].'取数失败,错误信息:'.$dataInfo['error'];
                        DataImportImp::saveDataErrorLog(1,SOURCE_ID,AD_PLATFORM,4,$error_msg);
                        $error_msg_arr[] = $error_msg;
                        CommonFunction::sendMail($error_msg_arr,AD_PLATFORM.'推广平台取数error');
                ...
                $error_msg = AD_PLATFORM.'推广平台'.$value['company_username'].'账号取数失败,错误信息:'.$appInfoList['error'];
                DataImportImp::saveDataErrorLog(1,SOURCE_ID,AD_PLATFORM,4,$error_msg);
                                $error_msg_arr[] = $error_msg;
                CommonFunction::sendMail($error_msg_arr,AD_PLATFORM.'推广平台取数error');
            }
