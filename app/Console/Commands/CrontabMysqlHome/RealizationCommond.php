...
# 变现数据分国家数据
class RealizationCommond extends Command
{
    # 变现数据分国家数据
    /**
     ...
     * @var string
     */
    protected $signature = 'RealizationCommond {beginday?} {endday?} ';
    ...
    public function handle()
    {
        ...
        try {
            ...
            // 异常报错
            $message = date("Y-m-d")."号,变现数据分国家数据程序报错,报错原因:".$e->getMessage();
            DataImportImp::saveDataErrorLog(5, 'pad-001', '变现数据分国家数据', 2, $message);
            $error_msg_arr[] = $message;
            CommonFunction::sendMail($error_msg_arr, '变现数据分国家数据');
            exit;
        }
    ...
    public function insertBasicDataHomePage($beginday,$endday){
        ...
        zplay_basic_realization_report_total
        WHERE
        ...
        //乘以0.78 国内  不乘0.78 国外
        $sql = "INSERT into zplay_basic_realization_report_total
SELECT 
 '' as id,
 a.date,
 c_app.id as app_id,
 ...
 sum(a.interst_ad_income) as interst_ad_income,
 sum(a.video_ad_incom) as video_ad_incom,
