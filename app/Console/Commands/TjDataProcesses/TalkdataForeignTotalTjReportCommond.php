...
class TalkdataForeignTotalTjReportCommond extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'TalkdataForeignTotalTjReportCommond {dayid?}';
    ...
    public function handle()
    {
        ...
        //先删除数据
        DB::delete("delete from talkingdata_foreign_channel where day='{$day}'");
...
//        $result = DB::select("select * from plat_tj_code where plat_id='ptj02'"); // todo 正式
        ...
        foreach ($result as $row){
            ...
            var_dump($gameinfo);
            sleep(3);
            TdDataFunction::foreignTotalCreateData($row['app_key'],$gameinfo,$row['platform_id'],$day,$groupby);
        }
    }
