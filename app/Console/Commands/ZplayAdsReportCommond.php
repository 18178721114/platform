...
class ZplayAdsReportCommond extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ZplayAdsReportCommond';
    ...
    public function handle()
    {
        ...
        if (!$result){
            echo "开发者账号{$developer_account_id}获取应用列表失败！";
            exit;
        }
        ...
        if ($result) {
            ...
            if (isset($ad_stats_data['error']) && $ad_stats_data['error'] == '' && isset($ad_stats_data['data']) && $ad_stats_data['data']) {
                ...
                if (isset($ad_stats['list']) && $ad_stats['list']) {
                    ...
                    $dbh = new \PDO('mysql:host=127.0.0.1;dbname=mydata', 'root', 'root2520');
                    $del = $dbh->exec('set names utf8');
                    foreach ($ad_stats_list as $key => $ad_stats) {
                        $ad_stats['ad_type'] = $ad_type;
                        $sql = "INSERT INTO `ad_report_data` (`date`, `app_id`, `ad_unit_id`, `play_start`, `play_finish`, `imp`, `click`, `request`, `income`, `country`, `currency`, `ad_type`) VALUES ({$ad_stats['date']},'{$ad_stats['app_id']}','{$ad_stats['ad_unit_id']}',{$ad_stats['play_start']},{$ad_stats['play_finish']},{$ad_stats['imp']},{$ad_stats['click']},{$ad_stats['request']},{$ad_stats['income']},'{$ad_stats['country']}','{$ad_stats['currency']}',{$ad_stats['ad_type']});";
                        var_dump($sql);
                        $del = $dbh->exec($sql);
                        //  数据库插入数据失败
                        if (!$del) {
                            echo "日期{$start_date}-{$start_date},保存{$app_id}应用的统计第{$page}页数据列表失败！";
                        }
                    }
                }
            }
        }else{
            echo "获取{$app_id}应用的统计第{$page}页数据列表失败！！";
        }
    }
