...
function download_file() {
        local url="https://hq.appsflyer.com/export/${1}/installs_report/v5?api_token=6efa7c19-07c0-4457-bfd4-67e5c19f0997&from=${2}&to=${2}"
        echo $url;
        curl -L "${url}" > "${file_path}/${1}_${2}.log"
        return ${?}
}

function download_organic_file() {
        local organic_url="https://hq.appsflyer.com/export/${1}/organic_installs_report/v5?api_token=6efa7c19-07c0-4457-bfd4-67e5c19f0997&from=${2}&to=${2}"
        echo $organic_url;
        curl -L "${organic_url}" > "${file_path}/${1}_${2}_organic.log"
        return ${?}
}
...
do
        ...
        file="${file_path}/${appkey}_${datetime}.log"
        echo $file
        ...
        # 自然量
        download_organic_file "${appkey}" "${datetime}"
        organic_file="${file_path}/${appkey}_${datetime}_organic.log"
        echo $organic_file
...
# 数据匹配处理
php artisan AfAnalysisIdfaCommond "${datetime}"